import { Flex, Text, Stack, Box, SlideFade, useColorModeValue } from "@chakra-ui/react";
import Link from "next/link";

export const Footer = () => {
  const borderColor = useColorModeValue("gray.200", "gray.700");
  const mutedColor = useColorModeValue("gray.500", "gray.300");
  const contactColor = useColorModeValue("gray.800", "white");
    ...
    <SlideFade in offsetY="10px" transition={{ enter: { duration: 0.6, delay: 1.8 } }}>
      ...
      >
        ...
        <Box textAlign={{ base: "center", md: "left" }}>
          <Text fontSize="12px" color={mutedColor} letterSpacing="1px" mb={2}>
          ...
          <Stack direction={{ base: "column", sm: "row" }} spacing={{ base: 1, sm: 4 }} align={{ base: "center", md: "flex-start" }}>
            ...
            <Text fontSize="12px" color={mutedColor} display={{ base: "none", sm: "block" }}>·</Text>
            <Text
              as="a"
        ...
        <Stack direction="row" spacing={6} align="center">
          {[
            ...
            <Link
              key={href}
              href={href}
              style={{ fontSize: "13px", color: navColor, opacity: 0.8, letterSpacing: "1px" }}
            >
              {label}
            </Link>
          ))}
